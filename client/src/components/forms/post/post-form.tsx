...
export const PostForm = () => {
  const [create, { data, loading, error }] = useMutation<CreatePostMutation, CreatePostMutationVariables>(CreatePostDocument);
  ...
  async function handleCreate(value: PostFormData) {
    ...
    if (response.data) console.log(response.data.createPost);
  }

  return (
    <div className="">
      <form className="flex flex-col gap-4" onSubmit={handleSubmit(handleCreate)}>
        <input className="border-2 border-gray-300 rounded-md h-full px-4 py-2" type="text" placeholder="Title" {...registerInput("title")} />
        {errors.title && touchedFields.title && <p>{errors.title.message}</p>}
        <textarea className="border-2 border-gray-300 rounded-md h-full px-4 py-2" placeholder="Content" {...registerInput("content")} />
